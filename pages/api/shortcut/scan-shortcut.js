// /pages/api/shortcut/scan-shortcut.js
...
// File này CHỈ dành cho SHORTCUT (Shortcuts app trên iPhone gọi thẳng vào
// đây để quét mã thanh toán POS). KHÔNG dùng requireAdmin — thay vào đó
// xác thực bằng SHORTCUT_KEY tĩnh, giống hệt create-p2p-shortcut.js.
// Nhánh admin (đăng nhập trang quản trị) nằm riêng ở /pages/api/momo/scan.js.
...
async function handlePosCharge(req, res) {
  ...
  const key = (req.body?.key || req.headers['x-shortcut-key'] || '').toString().trim()
  if (!process.env.SHORTCUT_KEY) {
    console.error('[scan-shortcut] Thiếu SHORTCUT_KEY trong env — chặn toàn bộ request để an toàn.')
    return res.status(500).json({ error: 'Server chưa cấu hình SHORTCUT_KEY' })
  }
  if (!key || key !== process.env.SHORTCUT_KEY) {
    return res.status(401).json({ error: 'Sai hoặc thiếu key' })
  }

  if (!PARTNER_CODE || !ACCESS_KEY || !SECRET_KEY) {
    console.error('[scan-shortcut][POST] Thiếu env: MOMO_PARTNER_CODE / MOMO_ACCESS_KEY / MOMO_SECRET_KEY')
  ...
  try {
    ...
    console.error('[scan-shortcut][POST] RSA Encrypt Error:', err.message)
  ...
  try {
    await redis.hset('momo:orders', {
      [orderId]: JSON.stringify({
        ...
        payType: '',
        paymentOption: '',
        source: 'pos-shortcut', storeId: store.id, storeName: store.name,
    ...
    console.log(
      `[scan-shortcut][POST] MoMo response: ${orderId}`,
      `resultCode: ${data.resultCode}`,
      `message: ${data.message}`
    )

    // Trả gọn cho Shortcut — chủ yếu cần biết thành công hay không.
    ...
    console.error('[scan-shortcut][POST] Server Error:', err)
    ...
    try {
      ...
      console.error('[scan-shortcut][POST] Redis update FAILED error:', redisErr)
    }
