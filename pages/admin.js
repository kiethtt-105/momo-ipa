...
const CSS = `
  :root { 
    ...
    --surface: rgba(255, 255, 255, 0.88); /* Tăng độ đục che nền rõ hơn, chống mỏi mắt */
  }
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .bg-mesh-container {
    ...
    min-height: 100vh;
    width: 100vw;
    background-color: #f3e9ed; /* Hạ tông nền chính tối hơn một chút để nổi bật màu loang */
    overflow-x: hidden;
  }
  
  /* TỐI ƯU HIỆU ỨNG: Rút ngắn thời gian di chuyển (6s - 8s) để màu loang nhanh và rõ hơn */
  .orb { 
    position: absolute; 
    border-radius: 50%; 
    filter: blur(60px); /* Giảm độ nhòe xuống để giữ cạnh khối màu sắc nét, rõ màu hơn */
    opacity: 0.65; /* Tăng độ đậm của màu */
    z-index: 0; 
    pointer-events: none; 
    transform: translate3d(0,0,0); /* Ép Render bằng phần cứng card đồ họa chống lag */
  }
  ...
  /* Tăng tốc độ render cho Dashboard */
  .dashboard { 
    padding-top: 85px; 
    position: relative; 
    z-index: 2; 
  ...
  .search-box input { padding: 9px 16px; border: 1px solid rgba(174, 0, 112, 0.15); border-radius: 10px; width: 260px; font-size: 14px; background: rgba(255, 255, 255, 0.7); font-family: inherit; transition: all 0.2s; }
  ...
  /* Cách ly Layer hộp thống kê để tăng hiệu năng di chuyển mượt mà */
  .stat-card { 
    ...
    box-shadow: 0 10px 30px rgba(174, 0, 112, 0.02);
    will-change: transform;
  }
  ...
  /* Cách ly Layer bảng dữ liệu giảm tải CPU */
  .table-container { 
    background: var(--surface); 
